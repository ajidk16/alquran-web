...
import type { BookmarkedVerse } from "./types"
  ...
  persist(
    (set, get) => ({
      ...
      addBookmark: (verse) => {
        ...
        const exists = bookmarks.some((b) => b.surahNumber === verse.surahNumber && b.verseNumber === verse.verseNumber)
        if (!exists) {
      ...
      setSelectedDuaCategory: (category) => set({ selectedDuaCategory: category }),
